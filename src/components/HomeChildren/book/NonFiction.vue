<template>
  <ul class="movies-list">
    <li v-for="n in nonfiction" :key="n.id">
      <router-link :to="{name:'BookView',params:{bkid:n.id}}">
        <div class="hotImg">
          <img :src="'https://images.weserv.nl/?url='+n.image" alt="电影图片" width="100" height="150">
        </div>
        <div class="hotTxt">
          <p>{{n.title}}</p>
...
<script>
  ...
  export default {
    ...
    created() {
      this.axios.get('/api/book/search?q=非虚构类&count=8')
        .then((res) => {
          let nonfiction = res.data.books
