<template>
  <ul class="movies-list">
    <li v-for="f in fiction" :key="f.id">
      <router-link :to="{name:'BookView',params:{bkid:f.id}}">
        <div class="hotImg">
          <img :src="'https://images.weserv.nl/?url='+f.image" alt="电影图片" width="100" height="150">
        </div>
        <div class="hotTxt">
          <p>{{f.title}}</p>
...
<script>
  ...
  export default {
    ...
    created() {
      this.axios.get('/api/book/search?q=虚构类&count=8')
        .then((res) => {
          let fiction = res.data.books
