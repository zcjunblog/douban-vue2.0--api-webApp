<template>
  <ul class="movies-list">
    <li v-for="t in travel" :key="t.id">
      <router-link :to="{name:'BookView',params:{bkid:t.id}}">
        <div class="hotImg">
          <img :src="'https://images.weserv.nl/?url='+t.image" alt="电影图片" width="100" height="150">
        </div>
        <div class="hotTxt">
          <p>{{t.title}}</p>
          <rating v-if="t.rating" :rating="t.rating"></rating>
        </div>
      </router-link>
    </li>
  </ul>
...
<script>
  ...
  export default {
    ...
    created() {
      this.axios.get('/api/book/search?q=旅行&count=8')
        .then((res) => {
          let travel = res.data.books
          this.travel = travel
          // console.log(hot)
        })
