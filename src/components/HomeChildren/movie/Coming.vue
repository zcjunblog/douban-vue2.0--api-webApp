<template>
  <ul class="movies-list">
    <li v-for="c in coming" :key="c.id">
      <router-link :to="{name:'MovieView',params:{id:c.id}}">
        <div class="hotImg">
          <img :src="c.images.small" alt="电影图片" width="100" height="150">
        </div>
        <div class="hotTxt">
          <p>{{c.title}}</p>
          <rating v-if="c.rating" :rating="c.rating"></rating>
        </div>
      </router-link>
    </li>
  </ul>
...
<script>
  ...
  export default {
    ...
    created() {
      this.axios.get('/api/movie/coming_soon?count=8')
        .then((res) => {
          let coming = res.data.subjects
          this.coming = coming
          // console.log(coming)
        })
