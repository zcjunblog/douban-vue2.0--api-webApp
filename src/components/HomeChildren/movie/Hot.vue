<template>
  <ul class="movies-list">
    <li v-for="h in hot" :key="h.id">
      <router-link :to="{name:'MovieView',params:{id:h.id}}">
        <div class="hotImg">
          <img :src="h.images.small" alt="电影图片" width="100" height="150">
        </div>
        <div class="hotTxt">
          <p>{{h.title}}</p>
          <rating v-if="h.rating" :rating="h.rating"></rating>
        </div>
      </router-link>
    </li>
  </ul>
...
<script>
  ...
  export default {
    ...
    created() {
      this.axios.get('/api/movie/in_theaters?count=8')
        .then((res) => {
