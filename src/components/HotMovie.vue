<template>
  <div>
    ...
    <div class="movie-content">
      <h1>影院热映</h1>
      <router-link class="img-list" :to="{name:'MovieView',params:{id:sub.id}}" v-for="(sub,index) in subjects"
                   :key="index">
        <div class="img">
          <img :src="sub.images.small" alt="">
          <div class="star">
            <p>{{sub.title}}</p>
            <rating v-if="sub.rating" :rating="sub.rating"></rating>
          </div>
        </div>
      </router-link>
    </div>
...
<script>
  ...
  export default {
    ...
    created() {
      this.axios.get('/api/movie/in_theaters?count=25')
        .then((res) => {
