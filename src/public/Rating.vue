<template>
  <div class="rating" v-if="rating">
    <template v-if="full === 0">
      <span class="rating0">暂无评分</span>
    </template>
    <template v-else>
      <template v-for="n in full">
        <span class="star-full"></span>
      </template><template v-for="n in half">
        <span class="star-half"></span>
      </template><template v-for="n in gray">
        <span class="star-gray"></span>
      </template>
    </template>
    <span class="average">{{parseFloat(rating.average).toFixed(1)}}</span>
  </div>
</template>
